...
export default function ImageLightbox({ images, index, onClose, onNext, onPrev }) {
    ...
    return (
        ...
        >
            ...
            {/* Image & Notes Wrapper */}
            <div
                className="relative inline-block max-w-[90vw] max-h-[90vh] group"
            ...
            >
                ...
                >
                    {({ zoomIn, zoomOut, resetTransform }) => (
                        <>
                            {/* Toolbar (Floating) */}
                            <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-1 p-1.5 bg-slate-900/80 backdrop-blur-md rounded-lg border border-white/10 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-50">
                                <ToolButton
                                    icon={<ZoomIn size={18} />}
                                    onClick={() => zoomIn(0.5)}
                                    title="Acercar"
                                ...
                                <div className="relative">
                                    ...
                                    {currentImage.notes && currentImage.notes.map(note => (
                                        <div
                                            ...
                                            className="absolute z-10 group/note"
                                        ...
                                        >
                                            ...
                                            {/* Tooltip / Popover */}
                                            <div className="absolute opacity-0 invisible group-hover/note:opacity-100 group-hover/note:visible 
...
function ToolButton({ icon, onClick, title }) {
    return (
        <button
            onClick={onClick}
            className="p-2 rounded-md transition-colors hover:bg-primary/20 text-slate-300 hover:text-primary"
            title={title}
        >
            {icon}
        </button>
